...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.diff_lambda = nn.Linear(self.n_embd, self.n_head, bias=True) if self.use_diff_attn else None
        ...
        if self.use_diff_attn:
            assert self.diff_attn_q2_source == "wo"

    def _project_q2(self, x):
        if self.diff_attn_q2_source == "wo":
            return (x @ self.c_proj.weight.T).reshape(x.shape[0], x.shape[1], self.n_head, self.head_dim)
        raise ValueError(f"Unsupported diff attention q2 source: {self.diff_attn_q2_source}")

    def __call__(self, x, ve, mask):
        batch_size, seq_len, _ = x.shape
        q = self.c_q(x).reshape(batch_size, seq_len, self.n_head, self.head_dim)
        q2 = self._project_q2(x) if self.use_diff_attn else None
        if q2 is not None and self.diff_attn_q2_blend < 1.0:
            q2 = self.diff_attn_q2_blend * q2 + (1.0 - self.diff_attn_q2_blend) * q
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        for block in self.blocks:
            ...
            if self.config.use_diff_attn and self.config.diff_attn_q2_source == "wo":
                if self.config.diff_attn_wo_init == "small_random":
                    block.attn.c_proj.weight = (
                        mx.random.uniform(
                            -self.config.diff_attn_wo_init_scale * scale,
                            self.config.diff_attn_wo_init_scale * scale,
...
def evaluate_bpb_with_fallback(model, tokenizer, batch_size):
    current_batch_size = batch_size
    while current_batch_size >= 1:
        try:
            return evaluate_bpb(model, tokenizer, current_batch_size), current_batch_size
        except RuntimeError as exc:
            if not is_eval_oom(exc) or current_batch_size == 1:
                raise
            next_batch_size = max(1, current_batch_size // 2)
            print()
            print(f"Final eval OOM at batch size {current_batch_size}; retrying with {next_batch_size}")
            gc.collect()
...
val_bpb, eval_batch_size = evaluate_bpb_with_fallback(model, tokenizer, FINAL_EVAL_BATCH_SIZE)
t_eval = time.time()
print(f"Final eval completed in {t_eval - t_train:.1f}s")
print(f"Final eval batch size used: {eval_batch_size}")
